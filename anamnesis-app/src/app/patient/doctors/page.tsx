    ...
    <div className="mt-1">
      ...
      <Card variant="gold" className="mb-4">
        ...
        <div className="text-[14px] text-tx2 mt-2.5">
          \u{1F4C5} 31 Mar 2026 · \u{1F550} 10:30 · Camera 204
        </div>
      ...
      <Card>
        ...
        <div className="flex gap-2 mb-4">
          {days.map((d) => (
            <div
              key={d.short}
              className={`flex-1 text-center py-2 rounded-rs border cursor-pointer transition-colors ${
                d.selected
                  ? 'bg-gold text-bg border-gold'
                  : 'bg-bg-surf text-tx3 border-brd hover:border-tx3'
              }`}
            >
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          {slots.map((s) => (
            <div
              key={s.time}
              className={`py-2 text-center rounded-rs border text-[13px] font-bold cursor-pointer transition-colors ${
                s.selected
                  ? 'bg-gold text-bg border-gold'
                  : s.off
                    ? 'bg-bg-surf text-tx3/40 border-brd cursor-not-allowed'
                    : 'bg-bg-surf text-tx2 border-brd hover:border-gold'
              }`}
            >
              {s.time}
            </div>
...
function ResearchPane() {
  ...
  const [expandedWithdraw, setExpandedWithdraw] = useState<string | null>(null);
  ...
  return (
    <div className="mt-1">
      ...
      <Card className="mb-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="flex-1" onClick={() => toggleDetail('s1')}>
            \u{1F4CB} Detalii studiu
          </Button>
          <Button
            variant="outline"
            size="sm"
            className="flex-1 !text-warn !border-warn/30"
            onClick={() => toggleWithdraw('s1')}
          >
            \u{1F6AA} Retragere
          </Button>
        </div>
        ...
        {expandedStudy === 's1' && (
          <div className="mt-3 pt-3 border-t border-brd">
            ...
            <div className="bg-bg-card border border-brd rounded-rs p-3 mb-2 space-y-2">
              {[
                { icon: '\u{1FA78}', label: 'Analize de sange — profil lipidic' },
                { icon: '\u{1FAC0}', label: 'Ecografie cardiaca + ECG' },
                { icon: '\u{1F48A}', label: 'Istoric medicatie cardiovasculara' },
                { icon: '\u{1F4CA}', label: 'Semne vitale — TA, AV, greutate' },
              ].map((item) => (
            ...
            <InfoBanner variant="gold" className="mb-2.5">
              <span className="text-[14px]">\u{1F512}</span>
        ...
        {expandedWithdraw === 's1' && (
          <div className="mt-3 pt-3 border-t border-brd">
            ...
            <Card className="mb-3">
              ...
              <div className="text-[13px] text-tx2 leading-relaxed space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <span>Datele deja colectate raman in studiu conform Art. 17(3)(d) GDPR — necesare integritatii stiintifice</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-ok flex-shrink-0">✓</span>
            ...
            <div className="text-[12px] text-tx3 mb-2 leading-relaxed">
              Perioada de procesare retragere: <strong className="text-tx">max. 30 zile</strong> de la confirmare. Vei fi notificat pe parcurs.
            </div>
            <Button variant="err" fullWidth>
              \u{1F6AA} Confirma retragerea din studiu
            </Button>
      ...
      <Card className="mb-3">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" className="flex-1" onClick={() => toggleDetail('s2')}>
            \u{1F4CB} Detalii studiu
          </Button>
          <Button
            variant="outline"
        ...
        {expandedStudy === 's2' && (
          <div className="mt-3 pt-3 border-t border-brd">
            ...
            <div className="bg-bg-card border border-brd rounded-rs p-3 mb-2 space-y-2">
              {[
                { icon: '\u{1FA78}', label: 'HbA1c + Glicemie in serie', badge: 'ok' as const },
                { icon: '\u{1F48A}', label: 'Medicatie antidiabetica', badge: 'ok' as const },
                { icon: '\u{1F4CA}', label: 'IMC + Circumferinta abdominala', badge: 'ok' as const },
                { icon: '\u{1F9EC}', label: 'Markeri genetici (panel metabolic)', badge: 'purp' as const },
              ].map((item) => (
                <div key={item.label} className="flex items-center gap-2.5">
                  <span className="text-[14px]">{item.icon}</span>
                  <div className="text-[13px] font-semibold text-tx flex-1">{item.label}</div>
        ...
        {expandedWithdraw === 's2' && (
          <div className="mt-3 pt-3 border-t border-brd">
            ...
            <InfoBanner variant="blue" className="mb-2.5">
              <span className="text-[14px]">ℹ️</span>
              <span className="text-[12px]">
                Conform contractului semnat, retragerea din registrul MetaboTrack necesita o perioada de notificare de <strong>60 zile</strong>, timp in care datele noi nu vor mai fi colectate. Retragerea nu afecteaza ingrijirea ta medicala.
              </span>
            </InfoBanner>
            <Card className="mb-3">
              <div className="text-[13px] font-bold text-tx mb-2">Ce se intampla la retragere:</div>
              <div className="text-[13px] text-tx2 leading-relaxed space-y-2">
                ...
                <div className="flex gap-2">
                  <span className="text-warn flex-shrink-0">⚠</span>
                  <span>Datele genetice deja procesate nu pot fi sterse (integritate registru — Art. 89 GDPR)</span>
                </div>
                <div className="flex gap-2">
                  <span className="text-ok flex-shrink-0">✓</span>
            ...
            <div className="text-[12px] text-tx3 mb-2 leading-relaxed">
              Perioada de procesare retragere: <strong className="text-tx">60 zile</strong> de la confirmare. Vei primi actualizari pe parcurs.
            </div>
            <Button variant="err" fullWidth>
              \u{1F6AA} Confirma retragerea din studiu
            </Button>
...
export default function PatientDoctorsPage() {
  ...
  return (
    <div>
      ...
      {activeTab === 'hub' && <HubPane onNavigate={setActiveTab} />}
      {activeTab === 'access' && <AccessPane />}
      {activeTab === 'book' && <BookPane />}
      {activeTab === 'research' && <ResearchPane />}
    </div>
  );
}
