...
const hemogram: LabRow[] = [
  { name: 'Hemoglobina', value: '14.2', ref: 'g/dL (13–17)', status: 'ok' },
  { name: 'Leucocite', value: '6.8', ref: '×10³/µL (4–10)', status: 'ok' },
  { name: 'Trombocite', value: '245', ref: '×10³/µL (150–400)', status: 'ok' },
  { name: 'VSH', value: '8', ref: 'mm/h (<15)', status: 'ok' },
];

const metabolic: LabRow[] = [
    ...
    <Card className="mb-3">
      ...
      <div className="space-y-0">
        {rows.map((row) => (
          <div
            key={row.name}
            className="flex items-center py-2 border-b border-brd last:border-b-0"
          >
            <div className="text-[13px] text-tx flex-1">{row.name}</div>
            <div
              className={`text-[15px] font-bold w-14 text-right ${
                row.status === 'hi'
                  ? 'text-err'
                  : row.status === 'lo'
                    ? 'text-warn'
  ...
  return (
    <div>
      ...
      <div className="text-[12px] text-tx3 text-center my-3.5">
        \u{1F7E2} Normal &nbsp; \u{1F7E1} Atentie &nbsp; \u{1F534} Critic
      </div>
      ...
      <Button variant="outline" fullWidth>
        \u{1F517} Verifica autenticitatea NFT
      </Button>
    </div>
  );
}
