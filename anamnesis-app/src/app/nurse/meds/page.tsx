...
function MedRowItem({ med }: { med: MedRow }) {
  const isDone = med.status === "done";
  const isPending = med.status === "pending";

  return (
    <div className={"flex items-center justify-between py-2.5 border-b border-brd last:border-b-0" + (isDone ? " opacity-60" : " cursor-pointer")}>
      <div className="flex-1 min-w-0">
        ...
        <div className="text-[11px] text-tx3 mt-0.5">
          ...
          {med.extra ? <span className="text-warn"> - {med.extra}</span> : null}
        </div>
      </div>
      <div className="flex-shrink-0 ml-2">
        {isDone ? <span className="text-[11px] text-ok font-bold">{"✓ " + med.time}</span> : null}
        {isPending ? <span className="text-[11px] text-warn font-bold">{"→"}</span> : null}
        {med.status === "upcoming" ? <span className="text-[11px] text-blue font-bold">{"→"}</span> : null}
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <Card className="mb-3 !border-l-[3px] !border-l-ok">
        ...
        {time0800.map(function (med, i) {
          return <MedRowItem key={i} med={med} />;
        })}
      </Card>
      ...
      <Button variant="outline" fullWidth className="mt-2">
        Semnaleaza problema la administrare
      </Button>
    </div>
  );
}
